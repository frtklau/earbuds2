...
fetch("earbuds_scores v2.xlsx")
    ...
    .then(buffer => {
        ...
        // Convert to JSON. range: 1 skips the initial metadata row ('s/n, weight...') 
        // making Row 2 (Earbuds, price.com...) the keys for the objects.
...
function renderList(items) {
    ...
    // If Excel maps empty rows, safely filter rows that don't have an earbud name
    const validItems = items.filter(item => item.Earbuds);
